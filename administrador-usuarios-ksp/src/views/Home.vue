...
export default {
  ...
  login() {
    ...
    axios.post('https://fvylzxqqv6.execute-api.us-east-1.amazonaws.com/dev/valida_credenciales', json)
      .then(data => {
        try {
          ...
          if (estado == 1)  {
            ...
            this.error = true;
            this.error_msg = user.Error;
          }
        } catch (error) {
          console.error(error);
          this.error = true;
          this.error_msg = "Error en el formato de la respuesta";
        }
      })
      .catch(error => {
        console.error(error);
        this.error = true;
        this.error_msg = "Error al enviar la solicitud";
      });
  }
}
